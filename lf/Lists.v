...
Example is_odd_test : is_odd 3 = true.
Proof. simpl. reflexivity. Qed.
...
Example non_removed: remove_one 9 [1;2;3] = [1;2;3].
Proof. simpl. reflexivity. Qed.

Example one_removed: remove_one 9 [1;2;3;9] = [1;2;3].
Proof. simpl. reflexivity. Qed.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | x::xs => if (x =? v) then (remove_all v xs)
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  ...
  | nil => true
  | x::xs => if (member x s2)
           then (included xs (remove_one x s2))
           else false
  end.
...
Proof. intros s v.
       simpl. replace (v =? v) with true.
       - rewrite add_comm. simpl. reflexivity.
       - induction v as [| m IHm'].
         + reflexivity.
...
Proof.
  intros l1 l2. induction l1 as [| m l IHl'].
