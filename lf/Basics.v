...
Compute (next_working_day tuesday).
(* ==> monday : day *)
...
Definition isred2(c: color) : bool :=
  match c with
  | primary red => true
  | _ => false
  end.

Compute isred2 black.
Compute isred2 (primary green).
Compute isred2 (primary red).
...
Example test_leb11:                leb1 2 2 = true.
Proof. simpl. reflexivity.  Qed.
Example test_leb22:                leb1 2 4 = true.
Proof. simpl. reflexivity.  Qed.
Example test_leb33:                leb1 4 2 = false.
Proof. simpl. reflexivity.  Qed.
...
Example test_ltb1:             (ltb 3 2) = false.
Proof. simpl. reflexivity. Qed.
...
Theorem mutl_1_l : forall n : nat, 0 * n = 0.
Proof.
  intros n. simpl. reflexivity. Qed.
...
Inductive test_O : Type :=
| Mon
| Tue
| Wed
.

Fixpoint next_test_O (n : test_O) :test_O :=
    match n with
    | Mon => Tue
    | Tue => Wed
    | Wed => Mon
...
Compute next_test_O Mon.

Theorem next_testing : forall b : test_O,
    next_test_O (next_test_O (next_test_O b)) = b.
...
(*
Fixpoint mod_O (a b f: nat) :nat :=
  match b, f with
  | 0,_ => 0
  | _, 0 => 0
  | _,_ =>
      match (ltb a b) with
      | false => mod_O (minus a b) b (minus f 1)
      | true => a
      end
  end.
*)
...
Theorem negation_fn_applied_twice :
  forall (f : bool -> bool ),
    ...
    forall (b : bool), f (f b) = b.
Proof. intros f H1 b. destruct b eqn:Eb.
       - rewrite -> H1. rewrite -> H1. simpl. reflexivity.
       - rewrite -> H1. rewrite -> H1. simpl. reflexivity.
Qed.
